...
public class Relation extends GridElement {
	...
	private static final Integer DIAMETRO_CIRCULO=15;
	String beginQualifier;
	String endQualifier;
	String beginArrow;
	String endArrow;
	 protected String beginMultiplicity;
	...
	protected Point getCenterOfLine() {
		Point ret = new Point();
	...
	public void setPanelAttributes(String state) {
		...
		System.out.println("cambio losatributos "+state);
		...
		for (int i = 0; i < tmp.size(); i++) {
			...
			if (s.startsWith("q1=") & (s.length() > 3)) {
				...
				if (s.indexOf("<(+)") >= 0) {
					...
					clientServer = "start"; // used for setting the startpoint
					// nonstickable
					beginArrow = "require2";//se usa para un atributo de clave primaria
				}
				else if (s.indexOf("<(") >= 0) {
				...
				if (s.indexOf("(+)>") >= 0) {
					...
					clientServer = "end"; // used for setting the endpoint
					// nonstickable
					endArrow = "require2";//se usa para un atributo de clave primaria
				}
				else if (s.indexOf(")>") >= 0) {
					clientServer = "end"; // used for setting the endpoint
					// nonstickable
	...
	public boolean lineUp(Vector<Rectangle> shapes, Vector<Point> points, int hotspotx, int hotspoty) {
		...
		for (int i = 0; i < shapes.size() - 1; i++) {
			...
			if (rMovable instanceof Arrow) {
				...
				else if (arrow.getString().equals("require")||arrow.getString().equals("require2")) {
					int size = (int) (DIAMETRO_CIRCULO * zoom);
					Point start = startIntersectingLine;
					Point end = endIntersectingLine;
			...
			if (r instanceof Qualifier) {
				...
				if (arrow.getString().equals("crowsFoot")) {
					...
					// A.Mueller Start
						...
						&& !arrow.getString().equals("require2")
				...
				else if (arrow.getString().equals("require2")) {

					int width = arrow.getCrossEndB().x - arrow.getCrossEndA().x;
					int height = arrow.getCrossEndB().y - arrow.getCrossEndA().y;
					g2.fillOval((int) arrow.getX() + arrow.getCrossEndA().x, (int) arrow.getY() + arrow.getCrossEndA().y, width, height);
