...
import com.umlet.element.Attribute;
...
public abstract class BaseGUI extends JPanel {
	...
	public OwnSyntaxPane createPropertyTextPane() {
		...
		JLabel propertyLabel = new JLabel(" Editar propiedades");
	...
	public JPopupMenu getContextMenu(Component comp) {
		...
		if (entity instanceof EER_Entity 
				|| entity instanceof EER_Rel_Diamond
				|| entity instanceof UseCase) {
			contextMenu.add(menuFactory.createDelete());
			if (entity instanceof EER_Entity){
				contextMenu.add(menuFactory.createAddIdElement());
				contextMenu.add(menuFactory.createAddComposedIdElement());
				contextMenu.add(menuFactory.createCambiarTipo());
			}
			contextMenu.add(menuFactory.createAddAttributeElement());
			if (!(entity instanceof UseCase))
				contextMenu.add(menuFactory.createAddComposedAttributeElement());
		}
		if (entity instanceof UseCase)
			contextMenu.add(menuFactory.createEditAttribute());
		
		if (entity instanceof EER_Entity)
			contextMenu.add(menuFactory.createGeneralizationElement());
		
		if (entity instanceof Specialization)
			contextMenu.add(menuFactory.createAddEntityToGeneralizationElement());
		
		if (entity instanceof EER_Rel_Diamond){
			contextMenu.add(menuFactory.createAddEntityToRelationUp());
			contextMenu.add(menuFactory.createAddEntityToRelationDown());
		}
		//TODO 
		...
		if (entity instanceof AttributePK) {
			contextMenu.add(menuFactory.createDelete());
			contextMenu.add(menuFactory.createEditAttribute());
		}
		if (entity instanceof RelationEntity) {
			contextMenu.add(menuFactory.createDelete());
			contextMenu.add(menuFactory.createEditRelationEntity());
		}
...
//	public abstract int getMailSplitPosition();
	...
	public abstract List<String> chooseFolderName();
	 public abstract void activarBotones();
	public abstract void actualizarArbol() ;
}
