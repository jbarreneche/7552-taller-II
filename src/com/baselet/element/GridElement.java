...
public abstract class GridElement extends JComponent {
	...
	public Point getStickingPoint(){
		return getStickingPoint(2);
	}
	
	public Point getStickingPoint(int point){
		StickingPolygon poly = null; 
		if (this.isStickingBorderActive()) 
			poly = this.generateStickingBorder(this.getX(), this.getY(), this.getWidth(), this.getHeight());
		else
			return null;
		if (point == 1)
			return poly.getLine(0).getP1();
		if (point == 2)
			return poly.getLine(0).getP2();
		else
			return null;
	...
	public final void drawStickingPolygon(Graphics2D g2) {
		...
		if (Utils.displaceDrawingByOnePixel()) 
			poly = this.generateStickingBorder(1, 1, this.getWidth() - 1, this.getHeight() - 1);
		else 
			poly = this.generateStickingBorder(0, 0, this.getWidth() - 1, this.getHeight() - 1);
		if (poly != null) {
			Color c = g2.getColor();
